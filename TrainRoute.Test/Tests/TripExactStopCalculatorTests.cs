...
{
    public class TripExactStopCalculatorTests
    {

        private readonly Dictionary<string, List<Route>> _routeMap;
        public TripExactStopCalculatorTests()
        {
        ...
        [Fact]
        public void CountTrips_ReturnsCorrectCount()
        {
            var calculator = new TripExactStopCalculator(_routeMap);

            var tripCount = calculator.CountTrips("A", "C", 4);

            Assert.Equal(3, tripCount); // ( A=>B=>C=>D=>C, A=>D=>C=>D=>C, A=>D=>E=>B=>C ) 
        }

        [Fact]
        public void CountTrips_NoValidRoute_ReturnsZero()
        {
            var calculator = new TripExactStopCalculator(_routeMap);
        ...
        [Fact]
        public void CountTrips_StartEqualsEnd_ReturnsCorrectCount()
        {
            var calculator = new TripExactStopCalculator(_routeMap);

            var tripCount = calculator.CountTrips("C", "C", 2);

            Assert.Equal(1, tripCount); // C=>D=>C
        }
    }
}
